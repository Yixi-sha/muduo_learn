...
int Udp_client(const char *hostname, const char *service, struct sockaddr **saptr, socklen_t *lenp, int family){
    ...
    while(addrinfoRet){
        ...
        if(clientFd > 0){
            *saptr = reinterpret_cast<struct sockaddr*>(malloc(addrinfoRet->ai_addrlen));
            if(*saptr == NULL){
...
struct IfiInfo* Get_ifi_info(int family, int doaliases){
    ...
    for(ptr = buf; ptr < buf + ifc.ifc_len;){
        ...
        if((flags & IFF_UP) == 0)
            continue;
        ifi = reinterpret_cast<struct IfiInfo *>(calloc(1, sizeof(struct IfiInfo)));
        if(ifi == NULL)
            err("calloc error");
        *ifipnext = ifi;
        ifipnext = &ifi->next;
        ...
        switch(ifr->ifr_addr.sa_family){
            case AF_INET:
                ifi->addr = reinterpret_cast<struct sockaddr*>(calloc(1, sizeof(struct sockaddr_in)));
                if(ifi->addr == NULL)
                    err("calloc error");
                ...
                if(flags & IFF_BROADCAST){
                    ...
                    ifi->baddr = reinterpret_cast<struct sockaddr*>(calloc(1, sizeof(struct sockaddr_in)));
                    if(ifi->addr == NULL)
                        err("calloc error");
                ...
                if(flags & IFF_POINTOPOINT){
                    ...
                    ifi->detaddr = reinterpret_cast<struct sockaddr*>(calloc(1, sizeof(struct sockaddr_in)));
                    if(ifi->addr == NULL)
                        err("calloc error");
            ...
            case AF_INET6:
                ifi->addr = reinterpret_cast<struct sockaddr*>(calloc(1, sizeof(struct sockaddr_in6)));
                if(ifi->addr == NULL)
                    err("calloc error");
                memcpy(ifi->addr, &ifr->ifr_addr, sizeof(struct sockaddr_in6));
                if(flags & IFF_POINTOPOINT){
                    ioctl(sockfd, SIOCGIFDSTADDR, &ifrcopy);
                    ifi->detaddr = reinterpret_cast<struct sockaddr*>(calloc(1, sizeof(struct sockaddr_in6)));
                    if(ifi->addr == NULL)
                        err("calloc error");
...
size_t Readline_r(int fd,void *vptr, size_t maxLen){
    ...
    if((rline = reinterpret_cast<Rline *>(pthread_getspecific(readLineKey))) == NULL){
        rline = reinterpret_cast<Rline *>(calloc(1, sizeof(Rline)));
